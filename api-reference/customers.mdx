...
<RequestExample>
```bash
curl -X GET https://api.cryptolay.com.br/api/customer/account \
...
<RequestExample>
```bash
curl -X PUT https://api.cryptolay.com.br/api/customer/account \
...
<RequestExample>
```bash
curl -X GET "https://api.cryptolay.com.br/api/customer/usage?period=month" \
  -H "X-API-Key: YOUR_API_KEY"
```
</RequestExample>
...
const getAccount = async () => {
  const response = await fetch('https://api.cryptolay.com.br/api/customer/account', {
    headers: {
...
const getUsage = async (period = 'month') => {
  const response = await fetch(
    `https://api.cryptolay.com.br/api/customer/usage?period=${period}`,
    {
      headers: {
        'X-API-Key': 'your_api_key'
      }
...
def get_account():
    response = requests.get(
        'https://api.cryptolay.com.br/api/customer/account',
...
def get_usage(period='month'):
    response = requests.get(
        f'https://api.cryptolay.com.br/api/customer/usage?period={period}',
        headers={'X-API-Key': 'your_api_key'}
    )
    return response.json()
```
